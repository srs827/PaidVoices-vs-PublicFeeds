...
# replace with proper paths for meta/bluesky from corresponding subfolder
ADS_WITH_CLUSTERS_PATH   = "clustering_code/ads_with_clusters.csv"
CLUSTER_THEMES_JSONL     = "coherency_check/cluster_themes.jsonl"

OUT_JSONL = "assigned_summaries.jsonl"
OUT_CSV   = "assigned_summaries.csv"
...
MAX_ADS_TO_CLASSIFY = None  # or set like 500 for testing

# debugging
DEBUG_VERBOSE         = True         # print detailed diagnostics to console
DEBUG_DUMP_TO_DISK    = True         # dump prompts and responses for each batch/retry
...
def normalize_summary_loose(s: str) -> str:
    s = normalize_summary(s)
    s = re.sub(r'[^a-z0-9 ]+', '', s)
    s = re.sub(r'\s+', ' ', s).strip()
    return s

def strip_md_quotes(s: str) -> str:
    """Remove quotes/backticks and **bold** markers often present in 'theme'."""
    s = (s or "").strip()
...
def load_cluster_summaries(jsonl_path: str) -> Tuple[List[Dict[str, Any]], Dict[int, str], Dict[int, str], set]:
    """
    JSONL lines like:
      {"cluster": 0, "summary": "...(long paragraph)...", "theme": "**\"Solar energy advocacy\"**", ...}

    returns:
      summaries_list: [{"cluster": int, "summary": str, "theme": str}, ...]
      summary_lookup: {cluster_id: summary_str}
      theme_lookup:   {cluster_id: theme_str_cleaned}
    ...
    with open(jsonl_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if not line:
                continue
            obj = json.loads(line)
            if "cluster" not in obj or "summary" not in obj or "theme" not in obj:
                continue

            cid = int(obj["cluster"])
            summary = str(obj["summary"]).strip()
            theme_raw = str(obj["theme"]).strip()
            theme_clean = strip_md_quotes(theme_raw)

            if summary == "" or theme_clean == "":
                continue

            summaries_list.append({"cluster": cid, "summary": summary, "theme": theme_clean})
            summary_lookup[cid] = summary
            theme_lookup[cid] = theme_clean

    kept_cluster_ids = set(summary_lookup.keys())
    summaries_list.sort(key=lambda r: r["cluster"])
    return summaries_list, summary_lookup, theme_lookup, kept_cluster_ids

def load_ads(csv_path: str) -> pd.DataFrame:
    """
    Required columns: cid, text, cluster
    """
    df = pd.read_csv(csv_path)
    required_cols = [id_col, text_col, "cluster"] # bluesky
    for col in required_cols:
        if col not in df.columns:
            raise RuntimeError(f"{csv_path} missing {col}")

    df[id_col] = df[id_col].astype(str) # ad_archive_id for meta
    df[text_col] = df[text_col].fillna("").astype(str).str.strip() # ad_creative_bodies for meta
...
def build_batch_prompt(ads_batch: List[Dict[str, str]], summaries_list: List[Dict[str, Any]]):
    """
    Ask the model to pick by NUMBER only.
    We list each option as: index. SUMMARY (summary truncated for brevity)
    """
    def _short(s, n=220):
        s = re.sub(r"\s+", " ", s.strip())
    ...
    summary_lines = []
    for idx, row in enumerate(summaries_list, start=1):
        summ = _short(row["summary"])
        summary_lines.append(f'{idx}. {summ}')

    ad_lines = []
    for idx, ad in enumerate(ads_batch, start=1):
        text_one_line = re.sub(r"\s+", " ", ad[text_col]).strip()
        ad_lines.append(f'{idx}. [ad_id={ad[id_col]}] {text_one_line}')

    k = len(ads_batch)
    if text_col == "ad_creative_bodies": # meta
        prompt = (
            "Choose, for each ad, the single best-fitting summary from the numbered list below.\n"
            "RETURN FORMAT (MUST follow exactly):\n"
            f"- Output EXACTLY {k} lines.\n"
            "- On line i, output ONLY the NUMBER (no text) of the chosen option for ad i.\n"
            "No words, no commas, no explanations—just the number per line.\n\n"
            "Options (summaries only):\n" + "\n".join(summary_lines) + "\n\n"
            "Ads:\n" + "\n".join(ad_lines) + "\n\n"
            "Response (numbers only, one per line):\n"
        )
    else: # bluesky
        prompt = (
            "Choose, for each post, the single best-fitting summary from the numbered list below.\n"
            "RETURN FORMAT (MUST follow exactly):\n"
            f"- Output EXACTLY {k} lines.\n"
            "- On line i, output ONLY the NUMBER (no text) of the chosen option for post i.\n"
            "No words, no commas, no explanations—just the number per line.\n\n"
            "Options (summaries only):\n" + "\n".join(summary_lines) + "\n\n"
            "Posts:\n" + "\n".join(ad_lines) + "\n\n"
            "Response (numbers only, one per line):\n"
...
def parse_llm_batch_response(
    ...
    """
    Prefer numeric indices (1..N) mapping to summaries_list.
    Fallback: try to match returned text to summary/theme via reverse_lookup.
    """
    ...
    # Build idx->cluster map
    idx_to_cluster = {str(i): summaries_list[i-1]["cluster"] for i in range(1, len(summaries_list)+1)}
    ...
    # Pad/truncate to number of ads so zip aligns
    ...
    for ad, raw_line in zip(ads_batch, lines):
        cleaned = _clean_label(raw_line or "")
        # 1) Try numeric index first
        m = re.match(r'^\s*(\d+)\s*$', cleaned) or re.match(r'^\s*(\d+)', cleaned)
        cid = None
        chosen_summary_text = None
        if m:
            idx = m.group(1)
            if idx in idx_to_cluster:
                cid = int(idx_to_cluster[idx])
                chosen_summary_text = summaries_list[int(idx)-1]["summary"]

        # 2) If not numeric or out of range, try matching text label/theme (fallback)
        if cid is None and cleaned:
            cleaned2 = strip_md_quotes(cleaned)
            cid_lookup = (reverse_lookup.get(normalize_summary(cleaned2)) or
                          reverse_lookup.get(normalize_summary_loose(cleaned2)))
            if cid_lookup is not None:
                cid = int(cid_lookup)
                chosen_summary_text = cleaned2

        if cid is not None:
            out[ad[id_col]] = {
                "chosen_cluster": cid,
                "chosen_summary": chosen_summary_text,
                "llm_label": raw_line,
                "needs_retry": False,
                "match_status": "matched",
                "cleaned_attempt": cleaned,   # diagnostics
            }
        else:
            out[ad[id_col]] = {
                "chosen_cluster": None,
                ...
                "cleaned_attempt": cleaned or None,  # diagnostics
            }
...
def print_batch_diagnostics(batch_idx: int,
    ...
    failed = [(ad[id_col], batch_map.get(ad[id_col], {})) for ad in ads_batch
              if not batch_map.get(ad[id_col], {}).get("chosen_cluster")]

    matched = [(ad[id_col], batch_map.get(ad[id_col], {})) for ad in ads_batch
               if batch_map.get(ad[id_col], {}).get("chosen_cluster")]
    ...
    for i, (ad_id, rec) in enumerate(failed, start=1):
        ...
        ad_text = next((a[text_col] for a in ads_batch if a[id_col] == ad_id), "")
        snippet = shorten(ad_text.replace("\n", " "), width=120, placeholder="…")


    # Show a couple matched examples with resolved theme (not shown to LLM)
...
def main():
    ...
    # Load themes (JSONL) and ads
    summaries_list, summary_lookup, theme_lookup, kept_cluster_ids = load_cluster_summaries(CLUSTER_THEMES_JSONL)
    print(f"[LOAD] {len(summaries_list)} clusters with themes")

    # Reverse lookup: allow matching by both summary and theme (used only in fallback, not shown to LLM)
    reverse_lookup: Dict[str, int] = {}
    for item in summaries_list:
        cid = int(item["cluster"])
        sum_txt = str(item["summary"])
        thm_txt = str(item["theme"])
        reverse_lookup[normalize_summary(sum_txt)] = cid
        reverse_lookup[normalize_summary_loose(sum_txt)] = cid
        reverse_lookup[normalize_summary(thm_txt)] = cid
        reverse_lookup[normalize_summary_loose(thm_txt)] = cid

    df = load_ads(ADS_WITH_CLUSTERS_PATH)
    print(f"[LOAD] {len(df)} ads/posts")
    ...
    for start in range(0, total, BATCH_SIZE):
        ...
        ads_batch = [{id_col: str(getattr(r, id_col)), text_col: str(getattr(r, text_col))} for r in batch_rows]

        batch_idx = start // BATCH_SIZE
        print(f"[BATCH] sending {len(ads_batch)} ads ({start}..{start+len(ads_batch)-1}) to LLM")
        prompt = build_batch_prompt(ads_batch, summaries_list)
        _dump_text("prompt", batch_idx, 0, prompt)

        try:
            raw = call_llm(prompt)
            _dump_text("response", batch_idx, 0, raw)
            batch_map = parse_llm_batch_response(raw, ads_batch, reverse_lookup, summaries_list)
            ...
            if str(e) == "RATE_LIMITED_429":
                ...
                print(f"[BATCH] fatal error for ads {start}-{start+len(ads_batch)-1}: {e}")
            break
        except Exception as e:
            print(f"[BATCH] error for ads {start}-{start+len(ads_batch)-1}: {e}")
            time.sleep(SLEEP_BETWEEN_BATCHES)
            continue

        # === Retry only failures ===
        id_to_text = {a[id_col]: a[text_col] for a in ads_batch}
        ...
        while failed_ids and retry_num < PER_BATCH_MAX_RETRIES and not hit_rate_limit:
            retry_num += 1
            print(f"[BATCH] retry #{retry_num}: reattempting {len(failed_ids)} failed ads")

            retry_ads_batch = [{id_col: ad_id, text_col: id_to_text[ad_id]} for ad_id in failed_ids]
            retry_prompt = build_batch_prompt(retry_ads_batch, summaries_list)
            ...
            try:
                ...
                retry_map = parse_llm_batch_response(raw_retry, retry_ads_batch, reverse_lookup, summaries_list)
    ...
    for r in df.itertuples(index=False):
        ad_id        = str(getattr(r, id_col))
        ad_text      = str(getattr(r, text_col))
        ...
        if not needs:
            ...
            if rec and rec.get("chosen_cluster") is not None:
                ...
                assign_reason = "llm_summary_reassign"
    ...
    reassigned_ok = sum(1 for row in final_rows if row["assign_reason"] == "llm_summary_reassign")
    ...
    print("=== SUMMARY ===")
    print(f"Total ads/posts: {len(df)}")
    print(f"LLM reassigned using summaries: {reassigned_ok}")
